...
function judge_freshman($input_sid)
{
	// ------ For beta testing
	//return true;
	return !strcasecmp("97", substr($input_sid, 0, 2));
}
...
function check_level($input_complete_opt, $input_all_opt)
{
	...
	elseif($c_percent < 0.15)
	{
		return 2;
	}
	elseif($c_percent < 0.28)
	{
		return 3;
	}
	elseif($c_percent < 0.41)
	{
		return 4;
	}
	elseif($c_percent < 0.54)
	{
		return 5;
	}
	elseif($c_percent < 0.67)
	{
		return 6;
	}
	elseif($c_percent < 0.80)
	{
		return 7;
	}
	elseif($c_percent >= 0.80)
