...
if($currmodule -> isadmin($curruser))
{
	if($_GET['action']=="editurl")
	{
		if(!empty($_GET['rwoID']))
		{
			$opt_list_url = $currdb -> query("SELECT * FROM `".($currdb -> prefix("regwizard_rel_links"))."` WHERE rwoID = ".$_GET['rwoID']."");
	...
	if($_GET['action']=="edit_single_url")
	{
		...
		{
			$opt_url = $currdb -> query("SELECT * FROM `".($currdb -> prefix("regwizard_rel_links"))."` WHERE rel_l_ID=".$_GET['rel_l_ID']."");
