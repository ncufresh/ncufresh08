...
if(!empty($_POST["edit_nav_done"]) && $currmodule->isadmin($curruser)){
	for($i=1;$i <= $_POST["biggest_csn"];$i++){
		$currdb->query("update `".$currdb->prefix("life_category")."` 
		...
		if($_POST["hsn-".$i] == 0){
			...
			$result_hsn_level = $currdb->query("select level from `".$currdb->prefix("life_category")."` where csn='".$_POST["hsn-".$i]."'");
			$hsn_level = $currdb->fetch_array($result_hsn_level);
			if($hsn_level["level"] == "1"){
				$currdb->query("update `".$currdb->prefix("life_category")."` set hsn='".$_POST["hsn-".$i]."' , level='2' where csn = '".$i."';");			
			}else{
				$currdb->query("update `".$currdb->prefix("life_category")."` set hsn='".$_POST["hsn-".$i]."', level='3' where csn = '".$i."';");
			}
		}
	}
...
if(!empty($_GET["new"]) && $currmodule->isadmin($curruser)){
	$result_test_last = $currdb->query("select csn from `".$currdb->prefix("life_category")."` order by csn desc");
	$test_last = $currdb->fetch_array($result_test_last);
	$last = $test_last["csn"];

	$result_test_allnums = $currdb->query("select csn from `".$currdb->prefix("life_category")."` where level='1'");
	$test_allnums = $currdb->num_rows($result_test_allnums);
	for($i=1;$i<=$_GET["new"];$i++){
		$last++;
		$test_allnums++;
		$currdb->query("insert into `".$currdb->prefix("life_category")."` (csn,title,hsn,ord,level,hyperlink) values ('".$last."','新分類','0','".$test_allnums."','1','index.php?csn=".$last."')" );
		$currdb->query("insert into `".$currdb->prefix("life_main")."` (csn,main) values ('".$last."','空的')");
	}
...
for($i = 1 ; $i <= $_POST["biggest_csn"] ; $i++){
	if($_POST["kill-".$i] != 1){
		continue;	}
	$currdb->query("delete from `".$currdb->prefix("life_category")."` where csn = '".$i."'");
	$currdb->query("delete from `".$currdb->prefix("life_main")."` where csn = '".$i."'");
...
$result_biggest_csn = $currdb->query("select max(csn) from `".$currdb->prefix("life_category")."`");
